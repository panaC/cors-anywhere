...
function withCORS(headers, request) {
  ...
  headers['access-control-expose-headers'] = Object.keys(headers).join(',') + ",X-CORS-Cookie-1"; // TODO hack, cf ln 192, onProxyResponse
    ...
    if (parsedLocation) {
      if (statusCode === 301 || statusCode === 302 || statusCode === 303) {
        ...
        if (requestState.redirectCount_ <= requestState.maxRedirects) {
          ...
          res.setHeader('X-CORS-Cookie-' + requestState.redirectCount_, proxyRes.headers['set-cookie'] || '')
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // passthrough x-cors-cookie to cookie
    // and in response set-cookie from 302 redirections 'x-cors-cookie-n' (n = [1,2,3,...]) and 'set-cookie' is preserved at final request 
    try {
      if (req.headers['x-cors-cookie']) {
        req.headers['cookie'] = req.headers['x-cors-cookie'];
      }
    } catch { }
